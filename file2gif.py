        ...
        if 'IN_CLOSE_WRITE' in event[1] and (EXTENSION in event[3] or EXTENSION2 in event[3] or EXTENSION3 in event[3]):
            ...
            try:
                ...
                ff = ffmpy.FFmpeg(
                    ...
                    outputs={gif_path: '-vcodec libx265 -crf 28 -ss 00:03:00.0 -to 00:03:30.0 -map 0:v -map 0:a:m:language:jpn? -map 0:a:m:language:eng? -map 0:a:m:language:pt? -map 0:s:m:language:eng? -map 0:s? -c:s copy -disposition:a:0 default -disposition:s:0 default'}
                )
                ff.run()
                ffs = ffmpy.FFmpeg(
